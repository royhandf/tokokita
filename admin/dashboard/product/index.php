<?php 
    ...
    $products = query(
                    "SELECT x.name AS category_name, y.*, z.* FROM categories x 
                    INNER JOIN product y ON x.id = y.category_id
                    INNER JOIN product_galleries z ON y.id = z.product_id GROUP BY z.product_id
                ");
?>
...
<body>
    <nav class="navbar navbar-dark bg-dark sticky-top p-3 shadow-sm">
        <div class="d-flex col-12 col-md-3 mb-2 mb-lg-0 justify-content-between">
            <a href="/dashboard" class="navbar-brand ps-md-4">Tokokita</a>
            ...
            <nav id="sidebar" class="col-md-3 col-lg-2 d-md-block bg-light collapse">
                <div class="pt-3 position-sticky">
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a href="../category/index.php" class="nav-link text-black">
                        ...
                        <li class="nav-item">
                            <a href="../customer/index.php" class="nav-link text-black">
                    ...
                    <div class="col-md-3">
                        <form action="" method="POST">
                            <div class="input-group">
                                ...
                                <button type="submit" name="search" class="input-group-text">
                    ...
                    <tbody>
                        ...
                        <?php foreach($products as $product) : ?>
                            <tr>
                                <td><?= $no++; ?></td>
                                <td>
                                    <a href="image.php?id=<?= $product['product_id']; ?>">
                                        <img src="../../../assets/<?= $product['image']; ?>" alt="<?= $product['image']; ?>" width="80">
                                    </a>
                                </td>
                                <td><?= $product['name']; ?></td>
                                ...
                                <td><?= $product['stock']; ?></td>
                                <td>
                                    <a href="info.php?id=<?= $product['product_id']; ?>" class="btn btn-info">Detail</a>
                                    <a href="update.php?id=<?= $product['product_id']; ?>" class="btn btn-warning">Update</a>
                                    <a href="delete.php?id=<?= $product['product_id']; ?>" class="btn btn-danger">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
